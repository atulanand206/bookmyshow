...
import com.creations.bms.exceptions.ServiceException;
import com.creations.bms.models.UserError;
...
public class ServiceExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  ResponseEntity<UserError> handleExceptions(final Exception ex, final WebRequest request) {
  ...
  ResponseEntity<UserError> handleBadRequest(final RuntimeException ex, final WebRequest request) {
    final var rootCause = getExceptionRootCause(ex);
    final var message = rootCause != null ? rootCause.getMessage() : "";
  ...
  private static Throwable getExceptionRootCause(@Nullable final Throwable cause) {
    ...
    if (rootCause == null) return cause;
    return rootCause;
  }

  private ResponseEntity<UserError> createErrorResponse(
      String message, WebRequest request, HttpStatus statusCode) {
  ...
  private UserError createError(String message, WebRequest request, HttpStatus status) {
    return new UserError(
        status.value(), message, Instant.now(), getUri(request), status.getReasonPhrase());
  }
